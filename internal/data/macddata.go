...
func NewMACD(list []*KLineMOne) *MACD {
...
func (e *MACD) Calculation() *MACD {
	...
	//临时变量，用于计算DEA
	var difTempKline []*KLineMOne
	for _, v := range e.points {
		difTempKline = append(difTempKline, &KLineMOne{StartTime: v.Time, EndPrice: v.DIF})
	}
	...
	for i := 0; i < len(e.points); i++ {
		...
		e.points[i].MACD = e.points[i].DIF - e.points[i].DEA
	}
	return e
}

// GetPoints return Point
func (e *MACD) GetPoints() []*biz.MACDPoint {
	points := make([]*biz.MACDPoint, 0)
	for _, v := range e.points {
		points = append(points, &biz.MACDPoint{
			Time: v.Time,
			DIF:  v.DIF,
			DEA:  v.DEA,
			MACD: v.MACD,
